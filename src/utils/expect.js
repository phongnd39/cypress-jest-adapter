...
const makeThrowingMatcher = (matcher, isNot, promise, actual, err) =>
	function throwingMatcher(...args) {	
		let throws = true;
		let expected = getExpected(arguments, matcher, isNot);
		...
		const processResult = (result, asyncError) => {
			...
			let passed = true;
			let message = getMessage(result.message);
			...
			message = formatMatcherMessage(message, isNot, promise, actual);

			// Custom assertion for use with Cypress
			var asr = new chai.Assertion(result.actual);
			asr.assert(passed, message, message, expected, getActual(actual, promise));
		};
