describe('Test Jest Syntax', function() {
  ...
  it('test toHaveLength', () => {
    ;([[[1, 2], 2], [[], 0], [['a', 'b'], 2], ['abc', 3], ['', 0]] as [
      any,
      number
    ][]).forEach(([received, length]: [any, number]) => {
      expect(received).toHaveLength(length)
    })
    ;([[[1, 2], 3], [[], 1], [['a', 'b'], 99], ['abc', 66], ['', 1]] as [
      any,
      number
  ...
  it('test toHaveProperty', () => {
    ...
    ;([
      ...
      [new Foo(), 'a', 'a'],
      [new Foo(), 'b', undefined]
    ] as [any, string, any][]).forEach(([obj, keyPath, value]) => {
    ...
    ;([
      ...
      [{ a: 0 }, 'a'],
      [{ a: { b: undefined } }, 'a.b']
    ] as [any, string][]).forEach(([obj, keyPath]) => {
